...
public class RobotContainer {
    ...
    // private final CommandSwerveDrivetrain m_commandSwerveDrivetrain = new
    ...
    // private static CommandXboxController joystick = new
    // CommandXboxController(Driver_Controller.SwerveCommandXboxControllerPort);// =
    // new CommandXboxController(0);

    // private final XboxController joystick2 = new
    // XboxController(Driver_Controller.SwerveRotaryEncoderPort);// = new
    // Joystick(1);

    public static final CommandSwerveDrivetrain drivetrain = TunerConstants.createDrivetrain();

    public static final SendableChooser<String> reefLevelChooser = new SendableChooser<>();
    public static final SendableChooser<String> reefPositionChooser = new SendableChooser<>();
    private final SendableChooser<Command> autoChooser;// = new SendableChooser<>();
    static AutoBuilder autoBuilder = new AutoBuilder();

    public RobotContainer() {

        reefPositionChooser.addOption("far right", "8");
        reefPositionChooser.setDefaultOption("far", "6");
        reefPositionChooser.addOption("far left", "4");
        reefLevelChooser.addOption("level 2", "2");
        reefLevelChooser.setDefaultOption("level 4", "4");
        ...
        SmartDashboard.putData("Reef position", reefPositionChooser);
        SmartDashboard.putData("Reef level", reefLevelChooser);
        // FollowPathCommand.warmupCommand().schedule();
    }

    public static double rotaryCalc(Boolean resetToRobot) {
        double pigeonYaw = drivetrain.getPigeon2().getYaw().getValueAsDouble(); // Read from the pigeon (Gyro angle as a
                                                                                // double) // Grab the yaw value from
                                                                                // the swerve drive IMU as a double
        ...
        // Return a slightly lowered powercurved value (i.e. Slightly lowered turn
        // speed)
    ...
    public static double joystick_curve(double joy) {
        ...
        for (i = 0; i < 10; i++) {
            if ((pos[i] <= joy) && (pos[i + 1] >= joy)) {
                // System.out.println( ((joy-pos[i]) / (pos[i+1]-pos[i]) * (pwr[i+1]-pwr[i]) +
                // pwr[i]) * MaxSpeed);
    ...
    private void configureBindings() {
        ...
        drivetrain.setDefaultCommand(
                ...
                drivetrain.applyRequest(() -> drive
                        .withVelocityX(Driver_Controller.SwerveXPassthrough
                                * ((Driver_Controller.SwerveCommandControl) ? autoDriveMultiplier : sliderAdjustment)) // Drive forward with negative Y (forward)
                        .withVelocityY(Driver_Controller.SwerveYPassthrough
                                * ((Driver_Controller.SwerveCommandControl) ? autoDriveMultiplier : sliderAdjustment)) // Drive left with negative X (left)
                        .withRotationalRate(rotaryCalc(false) * MaxAngularRate * TurnModifier * drivingOn
                                * ((Driver_Controller.SwerveCommandControl == true) ? 0.45 : 1)) // Drive counterclockwise with negative X (left)
                ));
